...
메서드란 $f(x)=2x+3$ 같은 (수학의) 함수와 비슷합니다. 이 함수는 x에 수를 대입하면 2배한 후 3을 더합니다. 당연한 얘기지만 숫자를 넣으면 숫자가 결과로 나옵니다. 메서드의 경우 $f(p)=c$에 더 가깝습니다. 입력으로 p를 대입하면 호출한 쪽에 c를 반환하죠.
...
> <details>
> <summary><a href="https://docs.unity3d.com/ScriptReference/MonoBehaviour.Awake.html">Awake</a>메서드는 public이 아니네요?</summary>
> 	<br><a href="https://docs.unity3d.com/ScriptReference/MonoBehaviour.Awake.html">Awake</a>를 비롯한 몇몇 메서드는 Unity 이벤트 메서드라고 부르며 특별취급합니다. 각 메서드를 알맞은 때에 Unity엔진에서 자동으로 호출해주며 어떤 형태로 선언되었는지 중요하지 않습니다.
> </details>
<br>
...
> <details>
> <summary>프로퍼티는 뭔가요?</summary>
> 	<br>프로퍼티는 쉽게 말해 필드같은 메서드입니다. 이는 읽기 전용 혹은 쓰기 전용으로 만들 수도 있습니다. 보통 C# 컨벤션은 프로퍼티를 대문자로 시작하지만 Unity는 소문자로 시작합니다.
> </details>
<br>
...
> <details>
> <summary>사원수는 뭔가요?</summary>
> 	<br>사원수(quaternion)는 3D 회전에 사용되는 복소수 기반 수체계입니다. X, Y, Z축 각도 회전보다는 이해하기 힘들지만 사원수만의 편한 성질들이 있습니다. 대표적으로 <b>짐벌락(gimbal lock) 현상</b>을 방지합니다.
> </details>
<br>
...
> <details>
> <summary>구조체는 또 뭔가요?</summary>
> 	<br>구조체(structure, 짧게는 struct)는 클래스같은 청사진입니다. 구조체의 경우 클래스와 다르게 객체가 아닌 integer나 color 같은 값으로 처리됩니다. 선언도 클래스와 동일하지만 class 대신 struct라고 쓰는 차이점만 있습니다.
> </details>
<br>
...
> <details>
> <summary>localRotation과 rotation은 무슨 차이가 있나요?</summary>
> 	<br>localRotation 프로퍼티는 Transform 컴포넌트만의 회전값을 제공합니다. Inspector뷰에서 볼 수 있는 회전값이 바로 이 프로퍼티이며 부모의 회전에 영향을 받습니다. 반면 rotation 프로퍼티는 월드 좌표 기준 모든 부모 계층의 회전값과 자신의 회전값(localRotation)을 적용한 최종 회전값을 나타냅니다. localRotation대신 rotation 프로퍼티의 값을 변경한다면 부모 계층의 회전값을 모두 적용하더라도 무시되고 설정한 rotation 프로퍼티의 값으로 반영됩니다.
> </details>
<br>

> <details>
> <summary><i>hoursPivot가 초기화되지 않았습니다.</i>라는 경고가 있으면 안 되죠?</summary>
> 	<br>컴파일러가 필드에 아무것도 할당되지 않은 것 처럼 생각하여 경고를 줄 수 있습니다. 우린 Unity Inspector뷰에서 할당했기 때문에 무시해도 되며 경고를 안 뜨게 막을 수도 있습니다. 상단 <i>Edit / Project Settings / Player / Other Settings / Script Compilation</i>에 <i>Suppress Common Warnings</i>라는 토글 버튼이 있습니다. 이걸로 위 경고나 private 접근 지정자를 쓰지 않았다는 경고를 안 뜨게 막을 수 있습니다.
> </details>
<br>
...
이제 매번 실행할 때마다 Console뷰에 현재 시스템 시간이 남을겁니다.
...
> <details>
> <summary>float이 뭔가요?</summary>
> 	<br>컴퓨터는 모든 수를 저장할 수 없기에 0과 1로 이루어진 비트로 이진 메모리에 저장합니다. 이 방법은 한정된 메모리 공간에 많은 수를 정확하게 저장할 수 없습니다. 예를 들어 $1\over 3$은 10진법으로 정확하게 표기하기 힘듭니다. 0.33333 이라고 끝없이 적다가 어느 지점에서 멈추는 방법 밖에는 없습니다.
> 	<br><br>
> 	소수를 소수점 이후 3자리, 앞에 1자리로 표현하기로 했다고 가정해봅시다. $1\over 3$은 근사치인 0.333 입니다. 이를 100으로 나누면 두 자리 만큼의 정확도를 잃은 0.003이라고 써야 하죠. 이런 작은 값들의 정확도를 높이기 위해 수의 크기를 나타내는 별도의 지수를 추가합니다. 그럼 정확도를 잃지 않고 $0.333 * 10^-2$ 처럼 표현할 수 있습니다. 100을 곱한 $0.333 * 10^2$도 마찬가지로 정확도가 유지됩니다. 그래서 소수점은 위치가 고정되지 않고 부동으로 간주됩니다. 이는 몇 자리 숫자로도 큰 수를 표현할 수 있게 합니다.
> 	<br><br>
> 	이런 부동 소수점도 10진수에서 2진수로 표현되는 점이나 NaN(not-a-number)과 무한이 따로 정의된 것을 제외하면 컴퓨터에서 동일하게 동작합니다. float 자료형은 그런 부동소수점을 저장하기 위한 4바이트 공간입니다.
> </details>
<br>
...
매 시가 각도로 변환될 때 곱해지는 값은 항상 동일합니다. 이런 hoursToDegrees를 ```const``` 접두사를 통해 값을 고정하도록 강제할 수 있습니다.
...
> <details>
> <summary>const 변수는 뭐가 다른가요?</summary>
> 	<br>const 키워드는 이 변수가 절대 변하지 않는 상수라는 것을 명시합니다. 컴파일 타임에 이 값으로 대체되며 숫자 같은 원초적인 타입에 적용할 수 있습니다.
> </details>
<br>
...
> <details>
> <summary>지역변수가 뭔가요?</summary>
> 	<br>필드와 동일하지만 지역변수가 포함된 메서드가 실행되는 동안에만 존재하며 메서드가 종료될 경우 메모리에서 해제됩니다. 필드와의 차이점은 클래스가 아니라 메서드에 귀속되는 점입니다.
> </details>
<br>
...
> <details>
> <summary>프레임이 뭔가요?</summary>
> 	<br>실행 중일 때 Unity는 Main Camera 오브젝트의 시점에서 끊임없이 화면을 렌더링합니다. 렌더링이 끝나면 화면에 그 결과가 표시되죠. 화면은 다음 프레임이 업데이트되고 렌더링되기 전까지 해당 프레임을 표시합니다. 그래서 Unity는 update, render, update, render...를 반복합니다. 그렇게 한 화면이 업데이트되고 렌더링되는 것을 1프레임이라고 하지만 실제로는 더 복잡합니다.
> </details>
<br>
...
완벽해보이지만 아쉽게도 *double에서 float으로 형변환할 수 없습니다.* 라는 컴파일 에러를 마주할 수 있을겁니다. 이는 [TimeSpan](https://learn.microsoft.com/ko-kr/dotnet/api/system.timespan?view=net-6.0)의 프로퍼티들이 일반 부동소수점이 아닌 **배정도 부동소수점(double-precision float)**으로 표현되기 때문입니다. 코드에선 double로 사용합니다. 하지만 Unity 코드들은 단정도 부동소수점(float)만 사용합니다.

> <details>
> <summary>단정도(single-precision)로 충분한가요?</summary>
> 	<br>대부분의 게임은 그렇습니다. 소수점 오류에 굉장힣 민감한 프로그램이 아니면 문제를 일으키지 않습니다. 
> </details>
<br>
